...
import com.wulewan.ghxm.utils.CrypticUtil;
...
import com.netease.wulewan.uikit.utils.NetworkEnvironmentUtil;
...
public class RequestHelp {
    ...
    public static void getRequest(String url, Object tag, Map<String, String> map, StringCallback callback) {
        if (!NetworkEnvironmentUtil.isNetworkConnected((Context) tag) && !NetworkEnvironmentUtil.isWifiConnected((Context) tag)){
            Toast toast = Toast.makeText((Context) tag,"无法连接网络，请检查WiFi或移动网络是否开启", Toast.LENGTH_LONG);
...
//                .cacheKey(url)//作为缓存的key
    ...
    public static void getReq(String url, Object tag, Map<String, String> map, StringCallback callback) {
        LogUtil.e("okgo", "getReq----->" + map.toString());
