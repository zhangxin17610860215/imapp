package com.netease.yqbj.uikit.business.team.helper;
...
public class TeamHelper {
     ...
     *
     * @param creator 群主
     * @param managerList 管理员列表
     * @return
     */
    public static ContactSelectActivity.Option getContactTeamManagerSelectOption(String creator,ArrayList<String> managerList) {
        ...
        if(managerList!=null){
            option.alreadySelectedAccounts = managerList;
        }
        return option;
    }
     ...
     *
     * @param teamId 群id
     * @param noCollarList 已被禁止的成员列表
     * @return
     */
    public static ContactSelectActivity.Option getContactNoCollarSelectOption(String teamId, ArrayList<String> noCollarList) {
        List<String> managerList = new ArrayList<>();
        List<String> allList = new ArrayList<>();
        String ownerId = "";
        allList.addAll(noCollarList);
        ...
        for (TeamMember member : teamMemberList){
            if (member.getType() == TeamMemberType.Owner){
                ownerId = member.getAccount();
            }
            if (member.getType() == TeamMemberType.Manager){
                managerList.add(member.getAccount());
            }
        }
        allList.add(ownerId);
        if (!NimUIKit.getAccount().equals(ownerId)){
        ...
        if(allList!=null){
            option.itemDisableFilter = new ContactIdFilter(allList);
        }
        return option;
    }
     ...
     * @return
     */
    public static ContactSelectActivity.Option getContactTeamSelectOption(List<String> memberAccounts,String creator) {
        ContactSelectActivity.Option option = new ContactSelectActivity.Option();
        option.maxSelectNum = 1;
        option.allowSelectEmpty = true;
        option.type = ContactSelectActivity.ContactSelectType.TEAM_MEMBER;
        option.title = "选择转让的用户";
...
//        option.maxSelectedTip = NimUIKit.getContext().getString(R.string.reach_team_member_capacity, teamCapacity);
    ...
    public static String getTeamMemberDisplayNameYou(String tid, String account) {
        if (!StringUtil.isEmpty(NimUIKit.getAccount()) && account.equals(NimUIKit.getAccount())) {
            return "你";
        }
