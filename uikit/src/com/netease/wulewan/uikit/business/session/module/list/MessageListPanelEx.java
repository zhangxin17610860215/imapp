package com.netease.wulewan.uikit.business.session.module.list;
...
import com.netease.wulewan.uikit.api.model.user.UserInfoObserver;
...
import com.netease.wulewan.uikit.business.session.activity.VoiceTrans;
import com.netease.wulewan.uikit.business.session.audio.MessageAudioControl;
import com.netease.wulewan.uikit.business.session.helper.MessageHelper;
import com.netease.wulewan.uikit.business.session.helper.MessageListPanelHelper;
import com.netease.wulewan.uikit.business.session.module.Container;
import com.netease.wulewan.uikit.business.session.viewholder.robot.RobotLinkView;
...
import com.netease.wulewan.uikit.common.ui.recyclerview.adapter.BaseFetchLoadAdapter;
import com.netease.wulewan.uikit.common.ui.recyclerview.adapter.IRecyclerView;
import com.netease.wulewan.uikit.common.ui.recyclerview.listener.OnItemClickListener;
import com.netease.wulewan.uikit.common.ui.recyclerview.loadmore.MsgListFetchLoadMoreView;
import com.netease.wulewan.uikit.common.util.media.BitmapDecoder;
import com.netease.wulewan.uikit.common.util.sys.ClipboardUtil;
import com.netease.wulewan.uikit.common.util.sys.NetworkUtil;
import com.netease.wulewan.uikit.common.util.sys.ScreenUtil;
import com.netease.wulewan.uikit.impl.NimUIKitImpl;
...
public class MessageListPanelEx {
    ...
    public void onMsgSend(IMMessage message) {
        ...
        if (msgType == MsgTypeEnum.custom){
            if (!message.getContent().contains("领取")){
                doScrollToBottom();
            }
        }else {
            doScrollToBottom();
...
//            refreshMessageList();
        ...
        @Override
        public void onClearMessages(List<IMMessage> messages) {
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private RequestCallback<List<IMMessage>> callback = new RequestCallbackWrapper<List<IMMessage>>() {
            ...
            public void onResult(int code, List<IMMessage> messages, Throwable exception) {
                ...
                if (messages != null) {
                    ...
                    if (messages.size() > 0){
                        lastTime = messages.get(messages.size() - 1).getTime() -1;
                    }
                    while (iterator.hasNext()) {
                        IMMessage imMessage = iterator.next();
                        if (null != imMessage.getAttachment()){
                            try {
                                Class aClass = Class.forName("com.wulewan.ghxm.session.extension.RedPacketOpenedAttachment");
                                //实例化一个RedPacketOpenedAttachment对象
                                Object obj = aClass.getConstructor().newInstance();
                                String attachmentName = imMessage.getAttachment().getClass().getName();
                                String redPacketOpenedAttachmentName = aClass.getName();
                                if (!TextUtils.isEmpty(attachmentName) && !TextUtils.isEmpty(redPacketOpenedAttachmentName)){
                                    if (attachmentName.equals(redPacketOpenedAttachmentName)){
                                        obj = imMessage.getAttachment();
                                        String sendAccount = (String) aClass.getMethod("getSendAccount").invoke(obj);
                                        String openAccount = (String) aClass.getMethod("getOpenAccount").invoke(obj);
                                        if (!TextUtils.isEmpty(sendAccount) && !TextUtils.isEmpty(openAccount)){
                                            if (!NimUIKit.getAccount().equals(sendAccount) && !NimUIKit.getAccount().equals(openAccount)){
                                                //与自己无关的红包领取气泡
                                                if (messages.size() > 1){
                                                    //判断是否是当前20条消息中最后一条与自己无关的红包领取气泡消息
                                                    iterator.remove();
                                                }
                                            }
                                        }
        ...
        private void loadFromRemote() {
            ...
            long aLong = instance.getLong(container.account);
            NIMClient.getService(MsgService.class)
                    .pullMessageHistoryEx(anchor(), aLong > 1 ? aLong : 0, loadMsgCount, direction,true)
                    .setCallback(callback);
        }
        ...
        private void onMessageLoaded(final List<IMMessage> messages) {
            ...
            boolean noMoreMessage = null != messages && messages.size() > 0 ? false : true;

//            boolean noMoreMessage = false;

//            SPUtils instance = SPUtils.getInstance(StatisticsConstants.ACCID);
//            long cleanTime = instance.getLong(container.account);
//            if (cleanTime > 1){
//                //用户点击清空历史消息
...
//                    while (iterator.hasNext()) {
//                        IMMessage imMessage = iterator.next();
//                        if (cleanTime > imMessage.getTime()){
//                            iterator.remove();
//                        }
//                    }
//                }else {
//                    noMoreMessage = null != messages && messages.size() > 0 ? false : true;
//                }
//            } else {
//                noMoreMessage = null != messages && messages.size() > 0 ? false : true;
//            }
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void showReDownloadConfirmDlg(final IMMessage message) {
            ...
            final Dialog dialog = EasyAlertDialogHelper.showCommonDialog(container.activity, null,
